...
print(calculateArea(x: -1, y: 2))
print(calculateArea(x: 3, y: 4))
...
func add(a: Int?, b: Int?) -> Int? {
    guard let first = a, let second = b else {return nil}
    return first + second
}
...
func createUser() -> User? {
    guard let first = firstNameTextField.text, let last = lastNameTextField.text, let age = ageTextField.text else {return nil}
